...
const skkmIconParts = function (iconParts, metadata) {
  ...
  const STROKE_RED = 'red'
  const STROKE_BLUE = 'blue'
  const STROKE_WHITE = 'white'
...
const skkmSIDCIcons = function (existingSIDCs, bbox, iconParts) {
  const sidc = {}
  sidc['K-G-D-----'] = [
    iconParts.AT_SKKM_TRIANGLE,
    iconParts.AT_SKKM_EXCLAMATION_MARK
...
const sanitize = value => {
  if (Array.isArray(value)) {
    ...
    if (value.type === 'translate') {
      return transform(value, [1, 0, 0, 1, 0, 0])
    ...
    replaceValue(value, 'fill', false, 'none')
    replaceValue(value, 'fill', '#123', 'rgb(17, 34, 51)')
    removeValue(value, 'font-family', 'Arial')
  }
