<script>
  ...
  const next = () => {
    ...
    if (state.index < upgrade.length - 1) {
      const index = state.index + 1
      progress.setWidthRatio(index / upgrade.length)
      iterations += 1
      ...
      state = {
        ...
        legacy: false,
        modern: false,
        legacySource: 'data:image/svg+xml;utf8,' + legacy(upgrade[index][0]).asSVG(),
        modernSource: 'data:image/svg+xml;utf8,' + legacy(upgrade[index][1]).asSVG()
      }
    ...
    if (state.legacy && state.modern) {
      ...
      if (difference > state.threshold) {
        review = [...review, {
          ...
          sidc_c: upgrade[state.index][0].sidc,
          sidc_d: upgrade[state.index][1].sidc,
          index: state.index
        }]
      }
...
<main>
  ...
  <div class='main'>
    ...
    <div class='status'>
      ...
      <span>progress: {state.index + 1} / {upgrade.length}</span>
    </div>
  </div>
  <div class='review'>
    {#each review as entry}
      <div class='card'>
        ...
        <div class='summary'>{entry.sidc_c}</div>
        <div class='summary'>{entry.sidc_d}</div>
      </div>
    {/each}
  </div>
</main>
